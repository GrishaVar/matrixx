...
class Plane(VectorSpace, Immutable):
    """
    2D Plane in N dimensional space implementation. 
    Is stored as a point and a normal vector, but can be constructed from that
    or from two lines in N dimensional space. 
    """

    _IS_VECTOR = True

    def __init__(self, point, normal vector):
        self.point = point
        self.normal = normal

        if self.point.size != self.nomral.size:
            raise ValueError(
                "Line source point and nomral have different dimensions"
            )

    def from_line_pair(self, line1, line2):
        self.
